...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager, step: int = None) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    ...
    try:
        if step is not None:
            reward_result = reward_fn(data, step=step, return_dict=True)
        else:
